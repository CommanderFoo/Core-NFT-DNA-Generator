...
|  |  | | | |     |  |  |  |   __| | | |   __|    -|     | | | |  |  |    -|
|____/|_|___|__|__|  |_____|_____|_|___|_____|__|__|__|__| |_| |_____|__|__|
...
The DNA Generator is a tool that can be used to generate content based on traits from NFTs. It can load in a specific
NFT, display a random NFT from a collection, and can also be used to build your own NFT by outputting the trait type and
trait value to the Event Log for you to easily use on OpenSea.

By creating the individual art assets and populating trait tables, you can generate content easily. A big benefit of this 
method of generation, is the fast iteration time.
...
Take a look at the options on the DNA Generator folder, and also look at the traits in the Traits folder to get an
idea on how this example was setup. Each trait in the traits folder will have a Table property, look at those tables
and see the columns and the type of data being stored.

--------------------------------
---   Understanding Traits   ---
--------------------------------

NFT traits are the building blocks that will be used to build up the final scene. For example, an Environment trait
would instruct the DNA Generator to look at the NFT attribute "Environment". Each trait also supports a value, so
an Environment trait could have a value of "Swamp". If there is an art asset linked to the "Environment" trait, then
it will be spawned in.

Imagine the different traits that could go with the Swamp environment. Tree type, water type, flowers, abandoned huts, etc.

Trait values can be matched against NFT traits, or they can be randomly used to generate random content that could then
be used for gameplay, or for generating your NFT which would involve taking a screenshot and adding the outputted
traits and values to your NFT on OpenSea.
...
There are 2 special objects that can be included in templates that the DNA Generator will look for.

- Spawn Points
  If spawn points exist in your template, then these will be used when spawning in the player. If there are multiple spawn points,
  one will be picked randomly.

- Unique Named Object
  The name of the object that will need to match the LocationName on a trait. If found, the template for that trait will be spawned
  at the position and rotation of the Named Object.
...
Traits are empty scripts that contain information in the custom properties about the trait.
...
- LocationName
  If the unique name is found inside the container holding the templates, then it will spawn the object at this location and
  rotation. This is useful if you need to spawn things such as resource nodes for players to mine.
...
All custom properties have detailed tooltips. Just hover over the custom property in the Properties window, and the tool 
tip will show up. If you are struggling to understand how to set up your traits, drag in the example project to see how that is set up.
...
- ShowWarnings
  Toggle on or off warnings in the Event Log. The DNA Generator will display warnings if it gets stopped (for example, missing item).
...
The base modifier script can be found in Project Content -> My Scripts -> DNA Generator. Add the DNA_Modifier script into 
the hierarchy as a child of an existing trait.
...
You may want some assets not to be modified when using modifiers. A custom property called DNAIgnore can be added to any
mesh in the Hierarchy, and the DNA Generator will ignore it.

-------------------------------
---   Included Components   ---
-------------------------------

The DNA Generator comes with various components that can be dropped into the Hierarchy. For an example of the compoments
